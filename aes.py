...
def sub_bytes(s, sbox=s_box):
    return [[sbox[i] for i in line] for line in s]
...
def add_round_key(s, k):
    return [[x ^ y for x,y in zip(sLine, kLine)] for sLine, kLine in zip(s,k)]
...
def decrypt(key, ciphertext):
    ...
    rkC = 10
    stateMat = bytes_to_matrix(ciphertext)
    stateMat = add_round_key(stateMat, round_keys[rkC])
    inv_shift_rows(stateMat)
    stateMat = sub_bytes(stateMat, inv_s_box)
    rkC -= 1
    for i in range(N_ROUNDS - 1, 0, -1):
        stateMat = add_round_key(stateMat, round_keys[rkC])
        inv_mix_columns(stateMat)
        inv_shift_rows(stateMat)
        stateMat = sub_bytes(stateMat, inv_s_box)
        rkC -= 1
    stateMat = add_round_key(stateMat, round_keys[rkC])
    return matrix_to_bytes(stateMat)
